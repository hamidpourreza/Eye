...
struct Options {
    ...
    int32_t optBatchSize = mxbs;
    // Maximum allowable batch size
    int32_t maxBatchSize = mxbs;
    // GPU device index
    int deviceIndex = 0;
};
...
public:
    ...
    bool runInference(const std::vector<std::vector<cv::cuda::GpuMat>>& inputs, std::vector<std::vector<cv::Mat>>& featureVectors);
    bool batchInference(const std::vector<std::vector<cv::cuda::GpuMat>>& inputs, std::vector<std::vector<std::vector<float>>>& featureVectors);
