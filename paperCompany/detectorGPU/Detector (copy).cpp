...
cv::Mat Detector::inference(cv::Mat cpuImg) {
    ...
    cv::Mat rightThresh, leftThresh,forThresh;
    cv::cvtColor(cpuImg,forThresh,cv::COLOR_BGR2GRAY);
    cv::threshold(forThresh, rightThresh, 220, 255, cv::THRESH_BINARY_INV);
    cv::threshold(forThresh, leftThresh, 150, 255, cv::THRESH_BINARY_INV);

    cv::destroyAllWindows();
    cv::Mat thresh=cv::Mat::zeros(rightThresh.size(),rightThresh.type());
    std::cout<<thresh.type()<<std::endl;

    rightThresh(cv::Rect(0, 0, 3000, rightThresh.rows)).copyTo(thresh(cv::Rect(0, 0, 3000, rightThresh.rows)));
    leftThresh(cv::Rect(3000, 0, leftThresh.cols - 3000, leftThresh.rows)).copyTo(thresh(cv::Rect(3000, 0, leftThresh.cols - 3000, leftThresh.rows)));
    ...
    cv::Mat processBatch(forThresh.size(), forThresh.type());
    ...
    int maxIdx;
        if (!nonZeroIndices.empty()) {
            ...
            cv::Mat currentData=cpuImg.colRange(minIdx+10,maxIdx-10).clone();
    ...
    cv::cuda::copyMakeBorder(img,padded,0,0,padd,padd,cv::BORDER_REFLECT);
    std::vector<cv::cuda::GpuMat> patches;
    std::vector<std::vector<cv::cuda::GpuMat>> batches;
//std::cout<<padded.size()<<std::endl;
    for(int h=0;h<img.rows-2*padd;h+=patchSize){
        for(int w=0;w<img.cols;w+=patchSize){
...
//    cv::merge(results, wholeRes);
    ...
    int stepH=img.rows/patchSize;
    int stepW=img.cols/patchSize;
...
//    std::cout << "post Started" << std::endl;
    ...
    mask.setTo(cv::Scalar(255, 0, 255), mask2);
    mask.setTo(cv::Scalar(0, 0, 255), mask);

    cv::addWeighted(mask, 1, zero, 0, 0, zero);
    cv::drawContours(zero, all, -1, cv::Scalar(255, 255, 255), cv::FILLED);
...
std::pair<float, float> Detector::calcSize(cv::Mat input, float scaleX, float scaleY) {
    std::pair<float, float> result;
    result.first=input.cols*scaleX;
