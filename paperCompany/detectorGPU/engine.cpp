...
bool Engine::runInference(const std::vector<std::vector<cv::cuda::GpuMat>> &inputs, std::vector<std::vector<cv::Mat>>& featureVectors) {
...
//    }
    ...
    for (int batch = 0; batch < batchSize; ++batch) {
        // Batch
        std::vector<cv::Mat> batchOutputs{};
        for (int32_t outputBinding = numInputs; outputBinding < m_engine->getNbBindings(); ++outputBinding) {
            ...
            cv::Mat result=cv::Mat((int)sqrt(outputLenFloat),(int)sqrt(outputLenFloat),CV_32FC1,output.data());
            batchOutputs.push_back(result.clone());
//            std::cout<<batchOutputs[0]<<std::endl;
        }
//        std::cout<<batchOutputs[0]<<std::endl;
...
bool Engine::batchInference(const std::vector<std::vector<cv::cuda::GpuMat>> &inputs, std::vector<std::vector<std::vector<float>>>& featureVectors) {
    // First we do some error checking
    if (inputs.empty() || inputs[0].empty()) {
        std::cout << "===== Error =====" << std::endl;
        std::cout << "Provided input vector is empty!" << std::endl;
